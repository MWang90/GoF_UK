...
series = read.csv("./ReadData/magzoneDK.csv", check.names = F, row.names = 1)
...
result = tibble("Scheme" = rep(Scheme.all$new[1:m], times = k), 
                "Agemodel" = rep(c("404kyr","100kyr","38kyr","21kyr"), each = m),
                "p" = NA)
...
ggsave((p1 | p2 | p3 | p4) + plot_layout(guides='collect'), filename = paste0("GoF_DK", Sys.time(), ".pdf"), width = 16, height = 8, dpi = 300)
