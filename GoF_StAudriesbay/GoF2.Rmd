...
series = read.csv("./ReadData/magzone_MW2.csv", check.names = F, row.names = 1)
n = nrow(series) # number of chron
k = ncol(series) # number of age model
```

# 2. Calculate GoF and completeness
```{r}
...
```{r}
set.seed(2)
mcn = 10000  # number of Monte Carlo
...
# 4. Save outputs
```{r}
write_rds(result, "./Outputs/result_2.rds")
write.table(result, "./Outputs/result_2.csv", row.names = F)
...
ggsave((p1 | p2 | p3 | p4) + plot_layout(guides='collect'), filename = paste0("GoF_MW2_", Sys.Date(), ".pdf"), width = 16, height = 8, dpi = 300)
```
